...
def run_command(script_path, config_file):
    command = ["python", script_path, "--config_file", config_file]
    try:
...
@click.command()
@click.option(
    "--config_file", help="Filepath relative to repo root of config", type=str
)
@click.option(
...
def main(config_file, skip_preprocess, skip_osmox):
